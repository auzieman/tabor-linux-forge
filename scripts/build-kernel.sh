...
BOOT_TARGETS=(zImage uImage modules dtbs)

if [[ -n "${KERNEL_UBOOT_IMAGE_TARGET}" ]]; then
  BOOT_TARGETS+=("uImage.${KERNEL_UBOOT_IMAGE_TARGET}")
fi

make -C "${KERNEL_DIR}" O="${OUT_DIR}" ARCH="${ARCH}" CROSS_COMPILE="${CROSS_COMPILE}" -j"${JOBS}" "${BOOT_TARGETS[@]}"
